...
import (
	"log"
	drivers "sabina/base/domain/drivers"
...
func (sApp *Application) SecurityServ() *SecurityService {
	return sApp.Services[tools.SecServ].(*SecurityService)
}

func (sApp *Application) DbServ() *MariaDBService {
	return sApp.Services[tools.DbServ].(*MariaDBService)
}

func (sApp *Application) GrpcServ() *GRPCService {
	return sApp.Services[tools.DbServ].(*GRPCService)
}
...
func NewApp(
	...
	gRPCServ = NewGRPCService(rApp.SrvsChan, pRPCPort, []ServiceDriver{
		*NewServiceDriver(
			...
			&UsrService_ServiceDesc),
		*NewServiceDriver(
			drivers.NewSyncServiceDriver(func() { rApp.CloseApp() }),
			&SyncService_ServiceDesc),
	})
